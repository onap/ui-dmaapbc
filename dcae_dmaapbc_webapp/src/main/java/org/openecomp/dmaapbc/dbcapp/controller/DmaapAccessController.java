...
public class DmaapAccessController extends DbcappRestrictedBaseController {
	 ...
	 * returns them in a JSON array nested within a response object.
	 * 
	...
	@ResponseBody
	public String getDmaapAccessList(HttpServletRequest request) throws ServletException {
		...
		try {
			...
			if (appUser == null || appUser.getLoginId() == null || appUser.getLoginId().length() == 0)
				throw new Exception("getDmaapAccessList: Failed to get  UID from request");
	 ...
	 * values for row ID and user ID in the body.
	 * 
	 * @param request
	...
	@ResponseBody
	public String addDmaapAccess(HttpServletRequest request) throws ServletException {
		MDC.put(SystemProperties.AUDITLOG_BEGIN_TIMESTAMP, DataBusHomeController.logDateFormat.format(new Date()));
		logger.setRequestBasedDefaultsIntoGlobalLoggingContext(request, DataBusHomeController.APP_NAME);
		String outboundJson = null;
		try {
			User appUser = UserUtils.getUserSession(request);
			if (appUser == null || appUser.getLoginId() == null || appUser.getLoginId().length() == 0)
				throw new Exception("addDmaapAccess: Failed to get  UID");
	...
	@ResponseBody
	public String updateDmaapAccess(@PathVariable("id") long id, HttpServletRequest request) throws ServletException {
		MDC.put(SystemProperties.AUDITLOG_BEGIN_TIMESTAMP, DataBusHomeController.logDateFormat.format(new Date()));
		logger.setRequestBasedDefaultsIntoGlobalLoggingContext(request, DataBusHomeController.APP_NAME);
		String outboundJson = null;
		try {
			User appUser = UserUtils.getUserSession(request);
			if (appUser == null || appUser.getLoginId() == null || appUser.getLoginId().length() == 0)
				throw new Exception("updateDmaapAccess: Failed to get  UID");
	...
	@ResponseBody
	public String deleteDmaapAccess(@PathVariable("id") long id, HttpServletRequest request) throws ServletException {
		MDC.put(SystemProperties.AUDITLOG_BEGIN_TIMESTAMP, DataBusHomeController.logDateFormat.format(new Date()));
		logger.setRequestBasedDefaultsIntoGlobalLoggingContext(request, DataBusHomeController.APP_NAME);
		String outboundJson = null;
		try {
			User appUser = UserUtils.getUserSession(request);
			if (appUser == null || appUser.getLoginId() == null || appUser.getLoginId().length() == 0)
				throw new Exception("deleteDmaapAccess: Failed to get  UID");
	...
	@ResponseBody
	public String getSelectedDmaapAccessProfile(HttpServletRequest request) throws ServletException {
		MDC.put(SystemProperties.AUDITLOG_BEGIN_TIMESTAMP, DataBusHomeController.logDateFormat.format(new Date()));
		logger.setRequestBasedDefaultsIntoGlobalLoggingContext(request, DataBusHomeController.APP_NAME);
		String outboundJson = null;
		try {
			User appUser = UserUtils.getUserSession(request);
			if (appUser == null || appUser.getLoginId() == null || appUser.getLoginId().length() == 0)
				throw new Exception("getSelectedDmaapAccessProfile: Failed to get  UID");
			...
			try {
				...
				throw new ServletException("getSelectedDmaapAccessProfile: Failed to decrypt password", ex);
			}
	 ...
	 * additionally an error message on failure.
	 * 
	...
	@ResponseBody
	public String selectDmaapAccess(@PathVariable("id") long id, HttpServletRequest request) throws ServletException {
		MDC.put(SystemProperties.AUDITLOG_BEGIN_TIMESTAMP, DataBusHomeController.logDateFormat.format(new Date()));
		logger.setRequestBasedDefaultsIntoGlobalLoggingContext(request, DataBusHomeController.APP_NAME);
		String outboundJson = null;
		try {
			User appUser = UserUtils.getUserSession(request);
			if (appUser == null || appUser.getLoginId() == null || appUser.getLoginId().length() == 0)
				throw new Exception("selectDmaapAccess: Failed to get  UID");
	...
	/**
	 * Tests the URL in the DMaaP access profile object.
	 * 
	 * @param request
	...
	@ResponseBody
	public String testDmaapAccess(HttpServletRequest request) throws ServletException {
		MDC.put(SystemProperties.AUDITLOG_BEGIN_TIMESTAMP, DataBusHomeController.logDateFormat.format(new Date()));
		logger.setRequestBasedDefaultsIntoGlobalLoggingContext(request, DataBusHomeController.APP_NAME);
		String outboundJson = null;
		try {
